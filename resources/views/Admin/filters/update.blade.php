    ...
    <div class="nk-block nk-block-lg">
        ...
        <div class="card card-bordered">
            <div class="card-inner">
                <form action="{{ url('admin-dashboard/update-filter/updateProcc') }}" class="form-validate"
                            ...
                            <div class="form-group">
                                ...
                                <div class="form-control-wrap">
                                    <input type="text" class="form-control" id="name" name="name"
                                        value="{{ old('name', $filter->translations->isNotEmpty() ? $filter->translations->first()->name : $filter->name) }}" />
                                </div>
                            </div>
                        </div>

                        <input type="hidden" class="form-control" id="slug" name="slug"
                            value="{{ old('slug', $filter->translations->isNotEmpty() ? $filter->translations->first()->slug : $filter->slug) }}" />
                        @if ($filter->translations->first()->language_id ?? '')
                            <input type="hidden" class="form-control" id="language_id" name="language_id"
                                value="{{ $filter->translations->first()->language_id ?? '' }}" />
                        @else
                            ...
                            <div class="form-group">
                                ...
                                <div class="form-control-wrap">
                                    <select class="form-select js-select2" name="category_id" data-search="on"
                                        @if ($languageRole !== 'global') disabled @endif>
                                    ...
                                    <div class="form-group row option-group mt-2">
                                        <div class="col-lg-10 col-md-10 col-sm-10">
                                            <input type="text" name="options[{{ $option->id }}]" class="form-control"
                                                ...
                                                value="{{ old('options.$option->id', $option->translations->isNotEmpty() ? $option->translations->first()->name : $option->name) }}">
