...
<div class="nk-block nk-block-lg">
    ...
    <div class="card card-bordered">
        <div class="card-inner">
            <form action="{{ url('admin-dashboard/filter/addProcc') }}" class="form-validate" novalidate="novalidate" method="post" enctype="multipart/form-data">
                @csrf
                <div class="row g-gs">
                    <!-- Name Field -->
                    <div class="col-md-6">
                                ...
                                @foreach (old('options') as $key => $option)
                                    <div class="form-group row option-group mt-2">
                                        <div class="col-lg-10 col-md-10 col-sm-10">
                                            <input type="text" name="options[]" class="form-control" placeholder="Enter option" value="{{ $option }}">
                                            @error("options.$key")
                                                <div class="error text-danger">{{ $message }}</div>
                                            @enderror
                                        </div>
